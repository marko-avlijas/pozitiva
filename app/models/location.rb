class Location < ActiveRecord::Base
  has_many :deliveries, inverse_of: :location
  ...
  def map_url
    ...
    "http://www.openstreetmap.org/?mlat=#{lat}&mlon=#{lng}#map=17/#{lat}/#{lng}" if lat.present? && lng.present?
  end
  ...
  def iframe_src
    ...
    "http://www.openstreetmap.org/export/embed.html?bbox=#{lng.to_d+0.06}%2C#{lat.to_d+0.025}%2C#{lng.to_d-0.06}%2C#{lat.to_d-0.0025}" + 
      "&amp;layer=mapnik&amp;marker=#{lat}%2C#{lng}" if lat.present? && lng.present?
  end
end
